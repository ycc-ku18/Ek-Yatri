...
public class ListSelectionPicker extends LinearLayout {
    ...
    public ListSelectionPicker setSelectedValues(ArrayList<String> values) {
        this.mValues = values;
        if (mValues != null) {
            if(mValues.size() == 1 && mValues.contains("")) {
                mSelectionView.setText(getContext().getString(R.string.select_empty));
            } else {
                mSelectionView.setText(TextUtils.join(", ", StringUtils.replaceNullOrEmptyByText(new ArrayList<>(values))));
            }
        } else {
